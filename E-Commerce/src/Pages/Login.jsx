...
const Login = () => {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    setLoading(true);
    // define admin credintails
    const AdminEmail = "admin123@gmail.com";
    const AdminPassword = "admin123";
    // check if admin credintails are correct
    ...
    try {
      ...
      setTimeout(() => {
        if (data.role === "admin") {
          navigate("/admin-dashboard"); // Redirect admin to the admin panel
        } else {
          navigate("/"); // Redirect normal users to the profile page
        }
      }, 1500);
    } catch (error) {
      ...
      <div className="md:flex md:w-1/2 h-full items-center justify-center mt-3.5">
        <div className="text-center max-w-md">
          <img
            src={img}
            alt="Illustration"
            className="w-80 mx-auto mb-6 rounded-2xl sm:mt-4"
          />
          <h2 className="text-2xl font-bold text-gray-800">
            Bride &<span className="text-green-600"> Groom Destination</span>
          </h2>
          <p className="text-gray-600 mt-2 pb-2.5">
            It is certainly important because it is only through hard work that
            we can achieve the goals of our life.
          </p>
        </div>
      </div>
          ...
          {message && (
            <div className="flex justify-center items-center mt-2">
              ...
              {loading && message.includes("Success") && (
                <FiLoader className="ml-2 text-green-500 animate-spin" /> // Loader icon with animation
              )}
            </div>
          )}
        </form>
